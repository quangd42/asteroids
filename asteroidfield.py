...
class AsteroidField(pygame.sprite.Sprite):
    edges = [
        ...
        [
            pygame.Vector2(-1, 0),
            lambda y: pygame.Vector2(
                const.SCREEN_WIDTH + const.ASTEROID_MAX_RADIUS, y * const.SCREEN_HEIGHT
            ),
        ],
        [
            pygame.Vector2(0, 1),
        ...
        [
            pygame.Vector2(0, -1),
            lambda x: pygame.Vector2(
                x * const.SCREEN_WIDTH, const.SCREEN_HEIGHT + const.ASTEROID_MAX_RADIUS
            ),
        ],
    ]
    ...
    def update(self, dt):
        self.spawn_timer += dt
        if self.spawn_timer > const.ASTEROID_SPAWN_RATE:
            self.spawn_timer = 0
